...
if __name__ == "__main__":
    ...
    nb_runs_CEM = args.nb_runs
    plots = []
    for run in range(1,1+nb_runs_CEM):
            ...
            while total_steps < args.max_steps:
                ...
                if step_cpt >= args.period:
                    ...
                    if(save_average_mu_in_csv):
                        ...
                        save_mu = open("average_mu_CEM_300_400.csv", 'a')
                        save_mu.write(str(total_steps)+","+str(f_mu)+"\r\n")#+ " : "+str(es.mu))
                        save_mu.close
    ...
    plt.figure()
    means = [np.mean([plots[i][iterat] for i in range(len(plots))]) for iterat in range(0,args.max_steps/args.period,1))]
    q25 = [np.quantile([plots[i][iterat] for i in range(len(plots))],0.25) for iterat in range(0,args.max_steps/args.period,1))]
    q75 = [np.quantile([plots[i][iterat] for i in range(len(plots))],0.75) for iterat in range(0,args.max_steps/args.period,1))]
    iters = list(range(args.period,args.max_steps+args.period,args.period))
    plt.plot(iters, means, c='b')
    plt.fill_between(iters,q25,q75, alpha=0.5)
    plt.show()
